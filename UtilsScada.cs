...
internal static class UtilsScada
{
    ...
    public static DateTime RoundToXMinutes(DateTime dt)
    {
        var minutos = 1;
        var ticksXmin = TimeSpan.FromMinutes(minutos).Ticks;
        return new DateTime((dt.Ticks + ticksXmin / 2) / ticksXmin * ticksXmin, dt.Kind);
    }
}